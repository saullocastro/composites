...
from composites.core import n_double_laminate
...
def test_ABD():
    ...
    phideg = 15
    psideg = 30
    matlamina = read_laminaprop(laminaprop, rho=0)
    lam = n_double_laminate(thickness, 2, np.asarray([phideg, psideg], dtype=np.float64), matlamina)
